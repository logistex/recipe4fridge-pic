...
### 5.2 선호 조건 (사용자 프로필)
- 요리 종류 (한식/양식/중식/일식 중 선택, 복수 선택 허용 여부는 디자인 시 결정)
...
profiles             id(=user_id, FK), cuisine_type, spice_level, difficulty, time_limit,
                     theme, is_admin(bool, default false), created_at
...
- 7.1의 후보 중 실제 구현 시점에 OpenRouter에서 사용 가능한 정확한 모델명 확정
- 이미지 자동 축소의 목표 해상도/용량 기준치 (예: 긴 변 1920px, 2MB 이하 등)
- 여러 장의 사진을 합치는 구체 방식 (그리드 이어붙이기 vs. 순차 전달 등 비전 모델 특성에 따라 조정 가능)
- 디자인 테마 3종의 구체적 컨셉 (색상/톤)
- 요리 종류 다중 선택 허용 여부
- 관리자 계정 부여 방식 (수동 DB 플래그 vs. 별도 초대 절차)
